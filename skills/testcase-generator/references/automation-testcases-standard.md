...
## 五、与 Midscene 自然语言 UI 用例的关系

- 若用户明确提到 `Midscene`、`自然语言 UI 自动化`、`根据设计稿生成自动化场景`，则在完成本文件的自动化候选筛选后，继续加载 `references/midscene-natural-language-testcases-standard.md`。
- Midscene 只适用于其中的 UI 场景；接口类、纯后端逻辑类自动化候选不需要强行转成自然语言页面步骤。
- 对适合 Midscene 的场景，输出时除保留自动化候选标记外，还应补充自然语言步骤、明确断言和必要的前置条件，便于后续落脚本。
